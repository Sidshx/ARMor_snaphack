...
# Screen grab setup
sct = mss.mss()
monitor = sct.monitors[1]  # Adjust index for multi-monitor setups
...
# Static screen image to avoid flickering
background_img = Image.frombytes('RGB', sct.grab(monitor).size, sct.grab(monitor).rgb)

# Background thread to refresh screen image every 2 seconds
def update_background():
    global background_img
    while True:
        img = sct.grab(monitor)
        background_img = Image.frombytes('RGB', img.size, img.rgb)
        time.sleep(2)
